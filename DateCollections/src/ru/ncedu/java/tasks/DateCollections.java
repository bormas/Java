...
/**
 * ЦЕЛЬ ЗАДАЧИ - научиться использовать коллекции из Java Collections Framework.
 * Дополнительно: освоить работу с датами ({@link Calendar}, {@link Date}, {@link DateFormat}),
 *  генерацию случайных чисел ({@link Random}).
 * <p/>
 * ЗАДАНИЕ
...
public interface DateCollections{
     ...
     */
    void setDateStyle(int dateStyle) throws ParseException;
    /**
    ...
    public static class Element {
        ...
        private String toString(Calendar date){
            return String.format("%2s/%2s/%4s", date.get(Calendar.DATE), date.get(Calendar.MONTH), date.get(Calendar.YEAR));
        }
    }
